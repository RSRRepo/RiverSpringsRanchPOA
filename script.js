...
printDocumentButton?.addEventListener("click", (event) => {
  event.preventDefault();
  event.stopPropagation();
  event.stopImmediatePropagation();

  const activeDocumentId = location.hash.slice(1);
  const activeDocument = documents.find(
    (documentRecord) => documentRecord.id === activeDocumentId
  );

  const pdfUrl =
    activeDocument?.pdf ||
    document.getElementById("open-pdf")?.getAttribute("href");
  ...
  /*
    Open the selected PDF in a separate tab only.
    Do not use window.location, location.href, or an iframe fallback here,
    because those would replace the SOP website in the original tab.
  */
  ...
  newTab.opener = null;
  newTab.location.href = new URL(pdfUrl, window.location.href).href;
